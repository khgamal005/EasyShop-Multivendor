...
import { getConversations, updateLastMessage } from "../redux/slices/conversationSlice";
...
const UserInbox = () => {
  ...
  const conversationIdFromURL = new URLSearchParams(location.search).get("conversationId");
    ...
    dispatch(getConversations({ id: user._id, role: "user" })).then((res) => {
      ...
      if (conversationIdFromURL) {
        ...
        if (matched) {
          ...
          axios.get(`${server}/shop/get-shop-info/${otherUserId}`)
            .then(res => setUserData(res.data.shop))
            .catch(console.error);
        }
      }
    });
...
const sendMessageHandler = async (e) => {
  ...
  const messageData = {
    ...
    images: [], // Keep if you want image support later
  };
  console.log("Sending message to backend:", messageData);


  try {
    // 1. Save message via backend
    ...
    if (savedMessage) {
      ...
      // 2. Emit saved message via socket
  ...
  const handleImageUpload = async (e) => {
    ...
    reader.onload = () => {
      if (reader.readyState === 2) {
        socket.emit("sendMessage", { senderId: user._id, receiverId, images: reader.result, text: "" });
      }
    };
    ...
    try {
      ...
      await dispatch(updateLastMessage({ id: currentChat._id, lastMessage: "Photo", lastMessageId: user._id }));
    } catch (err) {
...
const MessageList = ({ data, me, setOpen, setCurrentChat, setUserData, userData, online, setActiveStatus, loading }) => {
  ...
  useEffect(() => {
    ...
    axios.get(`${server}/shop/get-shop-info/${userId}`)
      .then(res => setUser(res.data.shop))
      .catch(console.error);
  }, [me, data]);

  return (
    <div className="w-full flex p-3 px-3 cursor-pointer hover:bg-[#00000010]" onClick={() => {
      setOpen(true);
      setCurrentChat(data);
      setUserData(user);
      setActiveStatus(online);
      ...
      <div className="relative">
        <img src={user?.avatar?.url} alt="avatar" className="w-[50px] h-[50px] rounded-full" />
      ...
      <div className="px-3 h-[75vh] py-3 overflow-y-scroll">
        {messages.map((item, index) => (
          <div key={index} className={`flex w-full my-2 ${item.sender === sellerId ? "justify-end" : "justify-start"}`} ref={scrollRef}>
            ...
            {item.images && <img src={item.images?.url || item.images} className="w-[300px] h-[300px] object-cover rounded-[10px] ml-2 mb-2" alt="" />}
            {item.text !== "" && (
              <div>
